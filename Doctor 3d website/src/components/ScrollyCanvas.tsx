...
export default function ScrollyCanvas() {
  ...
  const renderLoop = useCallback(function loop() {
    ...
    const isLoaded = (img: HTMLImageElement | undefined) => img && img.complete && img.naturalWidth > 0;

    let bestLowerImg = images[lowerIndex];
    if (!isLoaded(bestLowerImg)) {
      // Find nearest loaded frame searching backwards then forwards
      for (let offset = 1; offset < FRAME_COUNT; offset++) {
        const prev = lowerIndex - offset;
        const next = lowerIndex + offset;
        if (prev >= 0 && isLoaded(images[prev])) {
          bestLowerImg = images[prev];
          break;
        }
        if (next < FRAME_COUNT && isLoaded(images[next])) {
          bestLowerImg = images[next];
          break;
        }
      }
    }

    const bestUpperImg = images[upperIndex];

    if (isLoaded(bestLowerImg)) {
      // Draw base frame at full opacity
      drawImage(ctx, bestLowerImg as HTMLImageElement, clientWidth, clientHeight, 1);

      // If blending, overlay the next frame with fractional alpha
      if (blend > 0.01 && isLoaded(bestUpperImg) && bestLowerImg !== bestUpperImg) {
        drawImage(ctx, bestUpperImg, clientWidth, clientHeight, blend);
      }
    }
